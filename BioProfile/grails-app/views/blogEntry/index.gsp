		...
		<div id="list-blogEntry" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="datePublished" title="${message(code: 'blogEntry.datePublished.label', default: 'Date Published')}" />
				...
				<g:each in="${blogEntryList}" status="i" var="blogEntry">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${blogEntry.id}">${fieldValue(bean: blogEntry, field: "dateCreated")}</g:link></td>
			...
			<div class="pagination">
				<g:paginate total="${blogEntryCount ?: 0}" />
			</div>
		</div>
	</body>
</html>
