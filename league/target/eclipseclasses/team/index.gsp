		...
		<div id="list-team" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="coaches" title="${message(code: 'team.coaches.label', default: 'Coaches')}" />
					
						<g:sortableColumn property="roster" title="${message(code: 'team.roster.label', default: 'Roster')}" />
				...
				<g:each in="${teamInstanceList}" status="i" var="teamInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${teamInstance.id}">${fieldValue(bean: teamInstance, field: "coaches")}</g:link></td>
					
						<td>${fieldValue(bean: teamInstance, field: "roster")}</td>
